SamplesFile='M16s_SoilSamples.txt' #List of samples in tab delimited columns.
ShortFile=substr(SamplesFile,1,nchar(SamplesFile)-4)
EcotagFile='M16s_SoilSamples/Combined_Uniq_Sorted_Ecotag_CleanH.c10.txt'
# ShortFile="Combined_Uniq_Sorted_Ecotag_CleanH"
...
param$minlength=24 #Minimum length of sequence, pre-blast filtering (obitools and cutadapt). 20 for M16s, 50 for IN16STK and worms
...
print(cbind(as.character(Stats_Samples$SampleName[which(Stats_Samples$pc_Nr_Filtered<0.5)]),
  round(Stats_Samples$pc_Nr_Filtered[which(Stats_Samples$pc_Nr_Filtered<0.5)],digits = 3)),quote = F)
...
FA=read.table("FA2 Data.txt",header = T,sep="\t")
...
#obistat -c count M16s_SoilSamples/Combined_Uniq.fasta > Data_Stats/M16s_SoilSamples_Combined_Uniq.count
...
# Samples=Samples[-which(Samples$SampleName=="M16-PM-E7SS20"),]
...
RefSeq_Data=RefSeq_Data[which(RefSeq_Data$rank!="infraorder"),]
RefSeq_Data=RefSeq_Data[which(RefSeq_Data$rank!="no rank"),]
RefSeq_Data=RefSeq_Data[which(RefSeq_Data$rank!="superorder"),]
RefSeq_Data=RefSeq_Data[which(RefSeq_Data$rank!="parvorder"),]
...
RefSeq_Data_G=taxa_correction(RefSeq_Data_G,Seq_ID="HQ439494",action="ChangeTaxa",NewTaxa=c("genus","Equus"))
RefSeq_Data_G=taxa_correction(RefSeq_Data_G,Seq_ID=c("HQ439494","AB859014"),action="Group",colSamples=colSamples)
RefSeq_Data_G=taxa_correction(RefSeq_Data_G,Seq_ID="KY707308",action="ChangeTaxa",NewTaxa=c("genus","Peromyscus"))
RefSeq_Data_G=taxa_correction(RefSeq_Data_G,Seq_ID="AY122048",action="Remove")
RefSeq_Data_G=taxa_correction(RefSeq_Data_G,Seq_ID="KP698974",action="ChangeTaxa",NewTaxa=c("tribe","marmotini","NA"))
RefSeq_Data_G=taxa_correction(RefSeq_Data_G,Seq_ID="AC146433",action="Remove")
RefSeq_Data_G=taxa_correction(RefSeq_Data_G,Seq_ID="AB017708",action="ChangeTaxa",NewTaxa=c("genus","Homo","Homo"))
RefSeq_Data_G=taxa_correction(RefSeq_Data_G,Seq_ID=c("AB017708","AC021451","AC024248"),action="Group",colSamples=colSamples)
...
Samples$countPC=round(Samples$count/sum(Samples$count),digits = 2)
...
write.table(RefSeq_MOTUs,file = paste(substr(EcotagFile,1,nchar(EcotagFile)-4),"_RefSeq_Data.txt",sep=""),
            col.names = T,row.names = F,quote = F,sep = "\t")
RefSeq2Fasta(file_location=paste(substr(EcotagFile,1,nchar(EcotagFile)-4),"_RefSeq_Data.txt",sep=""),TRUE_Samples)
